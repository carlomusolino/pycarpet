...
def readData(data, job_id, me_ph, max_malloc, out_file_path="./data/carpetData.data", custom_header = None, endline=True, separator=" | "):
    '''
    Given the output data line e.g. extracted by readOutFile, takes the standard valuable data from it and writes it to a file, if the file doesn't yet exist it creates it. Standard data includes time of execution, maximum memory allocated, reconstruction method, and resolution. If more data is planned, one should pass a custom header. By default, it separates entries by a vertical line and puts an endline at the end of the data, behavior can be changed via endline and separator kewargs.
...
def extractCustomData(src,pattern, mode="line"):
    '''
Badly devised function, plan on removing asap. Now serves as a utility to extract data that is not easily isolated from the rest of the line via matching groups.
    '''
    if not type(src)==str: 
        try: line = src.group()
        except: raise ValueError("Line provided was not a match object")
